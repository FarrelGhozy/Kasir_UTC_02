...
// #startup-audit S9: bandingkan API key constant-time (hash SHA-256 dulu agar
// panjang selalu sama — timingSafeEqual menolak buffer beda panjang).
...
export const webhookRouter = new Elysia()
  ...
  .post("/api/v2/waha-webhook", async ({ headers, body, set }) => {
    try {
      ...
      if (!config.WAHA_API_KEY || !safeKeyEqual(key, config.WAHA_API_KEY)) {
        set.status = 401;
